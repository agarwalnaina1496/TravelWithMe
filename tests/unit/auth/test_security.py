...
def test_issue_jwt_resolves_to_the_same_user_id() -> None:
    ...
    token = issue_jwt(user_id, secret="test-secret", algorithm="HS256", expiry_days=1)

    assert verify_jwt(token, secret="test-secret", algorithm="HS256") == user_id
...
def test_verify_jwt_rejects_a_tampered_token() -> None:
    ...
    with pytest.raises(InvalidTokenError):
        verify_jwt(token, secret="a-different-secret", algorithm="HS256")
...
def test_verify_jwt_rejects_an_expired_token() -> None:
    ...
    with pytest.raises(InvalidTokenError):
        verify_jwt(expired, secret="test-secret", algorithm="HS256")


def test_verify_jwt_rejects_a_missing_subject_claim() -> None:
    now = datetime.now(timezone.utc)
    malformed = jwt.encode({"iat": now, "exp": now + timedelta(days=1)}, "test-secret", algorithm="HS256")

    with pytest.raises(InvalidTokenError):
        verify_jwt(malformed, secret="test-secret", algorithm="HS256")
