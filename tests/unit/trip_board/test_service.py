...
def test_atlas_supplied_leg_date_is_passed_through_as_trip_dates():
    trusted_action = FakeTrustedActionService()
    service = TripBoardService(trusted_action)
    final_itinerary = {
    ...
    assert leg.date_source == "trip_dates"


def test_atlas_supplied_leg_month_is_passed_through_as_trip_dates():
    service = TripBoardService(FakeTrustedActionService())
    final_itinerary = {
        "days": [_day(1, [_travel_item("Delhi", "Chennai", departure_month="2026-05")])],
    }

    board = service.build(TRIP_ID, 1, final_itinerary, {"origin_city": "Delhi"})

    leg = board.days[0].items[0]
    assert leg.date_precision == "month"
    assert leg.departure_month == "2026-05"
    assert leg.date_source == "trip_dates"


def test_legs_are_flexible_when_no_date_is_known():
    trusted_action = FakeTrustedActionService()
    service = TripBoardService(trusted_action)
    final_itinerary = {
        "days": [
...
def test_transport_search_pref_wins_over_an_atlas_supplied_leg_date():
    ...
    assert leg.date_source == "search_pref"


def test_transport_search_pref_applies_to_one_leg_only():
    service = TripBoardService(FakeTrustedActionService())
    final_itinerary = {
        "days": [
            _day(1, [_travel_item("Delhi", "Agra")]),
    ...
    board = service.build(
        TRIP_ID, 1, final_itinerary, {"origin_city": "Delhi"},
        _pref("transport", override_leg_id, date="2026-05-09"),
    )

    assert board.days[0].items[0].date_source == "none"
    assert board.days[0].items[0].departure_date is None
    assert board.days[1].items[0].departure_date == "2026-05-09"
    assert board.days[1].items[0].date_source == "search_pref"
...
def test_non_travel_item_never_gets_a_date_precision():
    ...
    assert board.days[0].items[0].date_source is None


def test_stay_segments_group_consecutive_same_location_stays():
    service = TripBoardService(FakeTrustedActionService())
    final_itinerary = {
        "days": [
    ...
    # No trip-level anchor: a stay with no search pref carries no dates.
    for segment in board.stay_segments:
        assert segment.date_precision == "flexible"
        assert segment.date_source == "none"
        assert segment.checkin_date is None
        assert segment.checkout_date is None
...
def test_search_pref_for_an_unknown_entity_is_inert():
    ...
    board = service.build(
        ...
        {
            "search_prefs": {
                "stays": {f"{TRIP_ID}:stay:9:9:nowhere": {"precision": "exact", "date": "2026-12-25"}},
                "transports": {f"{TRIP_ID}:99:0": {"precision": "exact", "date": "2026-12-25"}},
            }
        },
    )

    # The stale prefs never resolve to a live entity, so nothing carries
    # the 2026-12-25 date.
...
def test_stay_search_pref_sets_checkin_and_derives_checkout_from_nights():
    ...
    board = service.build(
        TRIP_ID, 1, final_itinerary, {"origin_city": "Delhi"},
        _pref("stay", segment_id, date="2026-06-10"),
    )
...
def test_stay_month_search_pref_carries_a_month_not_a_checkin():
    service = TripBoardService(FakeTrustedActionService())
    ...
    board = service.build(
        TRIP_ID, 1, final_itinerary, {"origin_city": "Delhi"},
        _pref("stay", segment_id, month="2026-05"),
    )

    segment = board.stay_segments[0]
    assert segment.date_precision == "month"
    assert segment.departure_month == "2026-05"
    assert segment.checkin_date is None
    assert segment.checkout_date is None
    assert segment.date_source == "search_pref"


def test_multi_city_stays_get_independent_checkins():
    service = TripBoardService(FakeTrustedActionService())
    final_itinerary = {
        "days": [
    ...
    jaipur_id = f"{TRIP_ID}:stay:1:1:jaipur"
    agra_id = f"{TRIP_ID}:stay:2:2:agra"
    ...
    board = service.build(
        TRIP_ID, 1, final_itinerary, {"origin_city": "Delhi"},
        {
            "search_prefs": {
                "stays": {
                    jaipur_id: {"precision": "exact", "date": "2026-06-10"},
                    agra_id: {"precision": "exact", "date": "2026-07-01"},
                }
            }
        },
    )

    by_location = {segment.location: segment for segment in board.stay_segments}
    assert by_location["Jaipur"].checkin_date == "2026-06-10"
    assert by_location["Agra"].checkin_date == "2026-07-01"
