...
Scout, Meridian, Guide, and Atlas emit readable checkpoints for HTTP receipt/response, validated input, agent invocation, raw adapter response, validation failure, validated output, normalized public response, and failures — through the shared `agent_engine` pipeline, so this applies identically regardless of which agent or which route (stateless `/scout`, `/meridian`, `/guide`, `/atlas`, or the Backend-owned `/trips/{id}/commands` boundary that Guide and Atlas also run through) invoked it. Invocation messages identify the selected engine and bounded traveler message. Failure messages identify the component, operation, stage, error type, and sanitized detail so FastAPI, n8n, and LangGraph failures are distinguishable without expanding the event. Each turn makes exactly one model invocation — there is no repair/regeneration retry — so `attempt` is always `1`.

## Correlation contract

Every route except the two health-check probes (`/`, `/health`) accepts optional `X-TWM-Request-ID`, `X-TWM-Trip-ID`, and `X-TWM-Turn-ID` headers — this is not limited to Scout and Meridian. Identifiers must start with an ASCII letter or digit, may then contain letters, digits, `.`, `_`, `:`, or `-`, and may contain at most 128 characters.
...
Context is held in an async-safe request-local variable and reset after the response, so concurrent requests do not share correlation data. Every log line emitted while handling one request shares that request's `request_id`, which is what makes one Axiom query able to reconstruct a full Scout, Meridian, Guide, or Atlas turn.
