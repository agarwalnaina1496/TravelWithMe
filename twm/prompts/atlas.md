...
- Every travel option and stay option carries `booking_readiness`: `suggested` for a general recommendation with no particular urgency, `needs_advance_booking` when the option is realistically time-sensitive or availability-limited, or `unresolved` when readiness cannot be determined. A day timeline item only carries `booking_readiness` when `requires_advance_booking` is true (a timed entry, permit, or transport leg that realistically needs advance action) — leave both absent otherwise. You never have visibility into a real reservation, so never mark anything as booked or confirmed; that status is set by the Backend once a traveler actually confirms it.
