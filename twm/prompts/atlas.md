...
- Put a fact in top-level `practical_notes` only when it applies to the trip as a whole rather than to one specific day. Do not repeat a fact verbatim or near-verbatim in both a day's `notes` and top-level `practical_notes`; choose the narrowest correct home.
