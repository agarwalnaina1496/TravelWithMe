...
Five facts use these exact fixed keys instead of an invented semantic key: departure origin under `origin_city`, party size under `num_travelers`, trip length under `trip_duration`, timing under `travel_dates`, and budget under `budget`. Preserve whatever form the traveler actually gave verbatim under `origin_city`, `num_travelers`, `trip_duration`, and `budget` — a range, "flexible", "not sure yet", a month, an approximate headcount. Give everything else a freely chosen semantic key, as usual.

Record `travel_dates` in plain ISO form when the traveler names a specific date, date range, or month — `2026-11-03`, `2026-11-03 to 2026-11-07`, or `2026-11` — dropping ordinal suffixes, articles, and filler ("the 3rd of November 2026" becomes `2026-11-03`). Given a day and month with no year, record just the day and month (`3 November`); never supply a year the traveler did not state. Keep the traveler's exact wording only when the timing is genuinely non-specific: a season, "flexible", "not sure yet", "a weekend sometime in spring". A firm timing constraint stated alongside the dates — a return-by date, a weekend-only window — is a distinct signal: keep it, in `travel_dates` or a semantic key of its own, never dropped.
