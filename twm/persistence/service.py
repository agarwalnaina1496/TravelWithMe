...
class TripPersistenceService:
    ...
    async def guest(self, request: Request, response: Response) -> GuestSession:
        ...
        guest = None
        if token:
            guest = await self.repository.resolve_guest(self._hash_token(token), self.settings.guest_session_days)
        if guest is None:
            token = secrets.token_urlsafe(32)
            guest = await self.repository.create_guest(self._hash_token(token), self.settings.guest_session_days)
        response.set_cookie(
