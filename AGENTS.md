...
| **Layer graph** — `routers → services → persistence → schemas` (+ `telemetry` / `trust_boundary` / `shared` cross-cutting); top imports down, never up. `routers/` see only `persistence.service` / `persistence.contracts`, not `persistence` internals; `schemas/` import no `services`/`persistence`; provider SDKs (`langchain*`, `langgraph`, `groq`) only in `services/agent_engine/` and `services/langgraph/`. | `import-linter` contracts in `pyproject.toml` `[tool.importlinter]` → `lint-imports` step in `ci-runner.yaml`; `tests/architecture/test_lint_config.py` guards the contract set |
