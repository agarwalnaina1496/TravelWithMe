...
| **Layer graph** — `routers → services → persistence → schemas` (+ `telemetry` / `trust_boundary` / `shared` cross-cutting); top imports down, never up. `routers/` see only `persistence.service` / `persistence.contracts`, not `persistence.postgres`; `schemas/` import no `services`/`persistence`; `persistence/` imports no `services/` (facts both layers share live in `twm.shared`). No `ignore_imports` exceptions. | `import-linter` contracts in `pyproject.toml` `[tool.importlinter]` → `lint-imports` step in `ci-runner.yaml`; `tests/architecture/test_lint_config.py` guards the contract set |
| **Provider-SDK boundary** — `langchain*` / `langgraph` / `groq` are imported directly only under `services/agent_engine/` and `services/langgraph/`; everything else reaches an agent through the `AgentEngine` protocol. | `scripts/check_provider_sdk_boundary.py` (direct-import check — import-linter traces transitively and can't express this) → step in `ci-runner.yaml` |
| **God-function caps** — cyclomatic complexity ≤ 12 (`C901`, ratchet target ~10), branches ≤ 15 (`PLR0912`), statements ≤ 60 (`PLR0915`). Three functions carry an inline `# noqa: C901` + reason (`schemas/flight_search.py` and `schemas/trusted_action.py` status-discriminator validators, `services/trip_view/trip_dates.py:compose_trip_dates`); the suppression list is capped at 5 by the config guard. | `ruff` config in `pyproject.toml` `[tool.ruff.lint]` → `ruff check twm scripts` step in `ci-runner.yaml`; `tests/architecture/test_lint_config.py` guards thresholds + the noqa count |
...
- **import-linter ratcheted debt** — every entry in a contract's `ignore_imports` is a known layering violation waiting to be removed; there should be none (the one from TWM-223 was removed in TWM-225).
