...
| **Layer graph** — `routers → services → persistence → schemas` (+ `telemetry` / `trust_boundary` / `shared` cross-cutting); top imports down, never up. `routers/` see only `persistence.service` / `persistence.contracts`, not `persistence.postgres`; `schemas/` import no `services`/`persistence`. One pre-existing exception is ratcheted in `ignore_imports` (`persistence.postgres → trip_commands.state`) — a follow-up removes it. | `import-linter` contracts in `pyproject.toml` `[tool.importlinter]` → `lint-imports` step in `ci-runner.yaml`; `tests/architecture/test_lint_config.py` guards the contract set |
