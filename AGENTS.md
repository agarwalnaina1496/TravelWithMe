...
| **Complexity + arity caps** — cyclomatic complexity ≤ 12 (`C901`, ratchet target ~10), branches ≤ 15 (`PLR0912`), statements ≤ 60 (`PLR0915`), positional args ≤ 6 (`PLR0913` — a 7th is a "pass an object" signal). | `ruff` config in `pyproject.toml` `[tool.ruff.lint]` → `ruff check .` step in `ci-runner.yaml`; `tests/architecture/test_lint_config.py` guards the thresholds |
